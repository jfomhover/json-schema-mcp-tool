...
def valid_minimal_doc():
    ...
    return {"title": "Test", "authors": ["Author"], "sections": []}
...
def valid_full_doc():
    ...
    return {
        ...
        "sections": [
            ...
            {"title": "Conclusion", "paragraphs": ["Final thoughts and summary."]},
        ],
    }
...
def invalid_doc_missing_title():
    ...
    return {"authors": ["Author"], "sections": []}
...
def test_create_document_with_valid_data(document_service, sample_schema):
    ...
    document = {
        "title": "Test Document",
        "authors": ["John Doe"],
        "sections": [{"title": "Introduction", "paragraphs": ["This is a test paragraph."]}],
    }
...
def test_create_document_metadata_correct(document_service, sample_schema):
    ...
    schema_id, _ = sample_schema
    document = {"title": "Metadata Test", "authors": ["Jane Smith"], "sections": []}
...
def test_create_document_id_is_ulid(document_service, sample_schema):
    ...
    schema_id, _ = sample_schema
    document = {"title": "ULID Test", "authors": ["Test User"], "sections": []}
...
def test_create_document_atomic_storage(document_service, sample_schema, temp_storage):
    ...
    schema_id, _ = sample_schema
    document = {"title": "Atomic Test", "authors": ["Atomic Author"], "sections": []}
...
def test_create_document_with_custom_id(document_service, sample_schema, temp_storage):
    ...
    document = {"title": "Custom ID Test", "authors": ["Custom Author"], "sections": []}
...
def test_create_document_custom_id_already_exists(document_service, sample_schema, temp_storage):
    ...
    custom_id = str(DocumentId.generate())
    document1 = {"title": "First Document", "authors": ["First Author"], "sections": []}
    document_service.create_document(schema_id, document1, doc_id=custom_id)

    # Try to create second document with same ID
    document2 = {"title": "Second Document", "authors": ["Second Author"], "sections": []}
...
def test_create_document_custom_id_must_be_valid(document_service, sample_schema):
    ...
    schema_id, _ = sample_schema
    document = {"title": "Test", "authors": ["Test Author"], "sections": []}
...
def test_create_document_missing_required_field(
    document_service, sample_schema, invalid_doc_missing_title
):
...
def test_update_node_updates_timestamp(
    document_service, sample_schema, valid_minimal_doc, temp_storage
):
...
def test_update_node_no_save_on_conflict(
    document_service, sample_schema, valid_minimal_doc, temp_storage
):
...
def test_create_node_add_to_object(document_service, sample_schema, valid_full_doc):
    ...
    new_section = {"title": "New Section", "paragraphs": ["New content"]}
    new_value, new_version = document_service.create_node(
...
def test_full_document_lifecycle(document_service, sample_schema, valid_full_doc):
    ...
    assert (
        sections_after_delete[0]["title"] == "Main Content"
    )  # First section is now "Main Content"
...
def test_concurrent_updates_version_conflict(document_service, sample_schema, valid_minimal_doc):
    ...
    with pytest.raises(VersionConflictError) as exc_info:
        document_service.update_node(doc_id, "/title", "Second Update", expected_version=1)
